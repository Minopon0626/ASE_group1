...
FORMAT_HITATI = "HITATI" # HITATIフォーマット
...
_T_HITATI = 500  # HITATIフォーマットの基準周期[us]
...
class Infrared:
    ...
    for frame in frames:
      ...
      if not first_frame:
        if ir_format == FORMAT_AEHA:
          ...
          code.append(self._round(100000 - t * t_count, 100))
      ...
      # Data
      if ir_format == FORMAT_AEHA or ir_format == FORMAT_NEC or ir_format == FORMAT_HITATI:
        for byte in frame:
          d = byte
          for i in range(8):
            bit = d & 1
  ...
  def decode(self, code):
    ...
    if self._cl(code[0], _T_AEHA * 8) and self._cl(code[1], _T_AEHA * 4):
      ...
      t = _T_SONY
    elif self._cl(code[0], _T_HITATI * 8) and self._cl(code[1], _T_HITATI * 16):
      ir_format = FORMAT_HITATI
      t = _T_HITATI
    else:
      return FORMAT_UNKNOWN, []
      ...
      for i in range(2, len(code), 2):
        ...
        if end_of_frame:
          ...
          if ir_format == FORMAT_AEHA:
            ...
            # Leader
            if self._cl(code[i], _T_HITATI * 8) and self._cl(code[i + 1], _T_HITATI * 16):
              end_of_frame = False
              continue
            # Repeat
            elif self._cl(code[i], _T_HITATI * 8) and self._cl(code[i + 1], _T_HITATI * 8):
              end_of_frame = False
              continue
            else:
              return FORMAT_UNKNOWN, []
